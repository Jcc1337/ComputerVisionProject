...
class moments(Function):
    ...
    def forward(ctx, x):
        if x.is_cuda:
            ex, ex2 = src.gpu.expectation_forward(x)
        else:
    ...
    def backward(ctx, x, dex, dex2):
        if x.is_cuda:
            dx = src.gpu.expectation_backward(x, dex, dex2)
        else:
            raise NotImplemented
        return dx
...
class syncbatchnorm_(Function):
    ...
    def forward(cls, ctx, x, gamma, beta, running_mean, running_var,
        ...
        if x.is_cuda:
            y = src.gpu.batchnorm_forward(x, _ex, _exs, gamma, beta, ctx.eps)
        else:
            y = src.cpu.batchnorm_forward(x, _ex, _exs, gamma, beta, ctx.eps)
    ...
    def backward(ctx, dz):
        ...
        if dz.is_cuda:
            dx, _dex, _dexs, dgamma, dbeta = \
                src.gpu.batchnorm_backward(dz, x, _ex, _exs, gamma, beta, ctx.eps)
        else:
            raise NotImplemented
        ...
        if ctx.training:
            ...
            if x.is_cuda:
                dx_ = src.gpu.expectation_backward(x, _dex, _dexs)
            else:
                raise NotImplemented
            dx = dx + dx_
...
def _act_forward(ctx, x):
    if ctx.activation.lower() == "leaky_relu":
        if x.is_cuda:
            src.gpu.leaky_relu_forward(x, ctx.slope)
        else:
            raise NotImplemented
    else:
...
def _act_backward(ctx, x, dx):
    if ctx.activation.lower() == "leaky_relu":
        if x.is_cuda:
            src.gpu.leaky_relu_backward(x, dx, ctx.slope)
        else:
            raise NotImplemented
    else:
        assert ctx.activation == 'none'
...
class inp_syncbatchnorm_(Function):
    ...
    def backward(ctx, dz):
        ...
        if dz.is_cuda:
            dx, _dex, _dexs, dgamma, dbeta = \
                src.gpu.batchnorm_inp_backward(dz, z, _ex, _exs, gamma, beta, ctx.eps)
        else:
            raise NotImplemented
